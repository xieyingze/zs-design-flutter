...
class DioLogger extends Interceptor {
  ...
  Future onResponse(Response response, ResponseInterceptorHandler handler) async {
    ...
    if (responseBody) {
      ...
      logPrint('║requestTime:${DateTime.now().difference(timeRequest).inMilliseconds}');
      logPrint('║');
      _printLine('╚');
    }
...
//      key = '\"$key\"';
      ...
      if (value is Map) {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          for (int i = 0; i < lines; ++i) {
            logPrint('║${_indent(tabs)} ${i == 0 ? '$key:' : ''}  ${msg.substring(i * linWidth, math.min<int>(i * linWidth + linWidth, msg.length))}');
          }
        } else {
